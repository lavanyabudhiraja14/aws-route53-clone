...
def list_records(
    ...
    search: str | None = Query(
        default=None,
        description="Filter by record name, type, or value",
    ),
...
def delete_records_bulk(
    ...
    zone = get_zone_or_404(
        db,
        zone_id,
    )

    record_ids = list(
        dict.fromkeys(payload.record_ids)
    )
    ...
    if missing_ids:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="One or more DNS records were not found in this hosted zone",
        )
    ...
    return MessageResponse(
        detail=f"{len(records)} DNS records deleted"
    )
